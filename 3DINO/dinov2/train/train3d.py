...
def do_train(cfg, model, resume=False):
    ...
    #         # load the .pt files they are WxHXD tensors
    #         image_dict["image"] = torch.load(image_dict["image"])
    #         # if len(image_dict["image"].shape) == 3:
    #         image_dict["image"] = image_dict["image"].unsqueeze(0)
    #         return image_dict
    ## AA CHANGE END
    ...
    data_transform = Compose(
            [   
                ...
                # MyLoadImage(),
                Lambdad(keys=['image'], func=lambda x: torch.load(x).unsqueeze(0)),
