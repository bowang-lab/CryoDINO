...
## AA CHANGES
class CryoMetrics4(BTCVMetrics):
    """Metrics for 4-class CryoET segmentation (e.g., CZII datasets)."""

    def __init__(self):
        super().__init__()
        self.post_label = AsDiscrete(to_onehot=4)
        self.post_pred = AsDiscrete(argmax=True, to_onehot=4)
class CryoMetrics6(BTCVMetrics):
    """Metrics for 6-class CryoET segmentation (e.g., CZII datasets)."""

    def __init__(self):
        super().__init__()
        self.post_label = AsDiscrete(to_onehot=6)
        self.post_pred = AsDiscrete(argmax=True, to_onehot=6)
...
def get_metric(dataset_name):
    if dataset_name == "BTCV":
        ...
        # Binary segmentation for EMPIAR 10989 and CZII 10010
