# Author: Tony Xu
# Adapted to CryoET by Ahmadreza Attarpour
...
def get_args_parser():
    ...
    parser.add_argument(
        "--image-size",
        help="Size of image to visualize",
        type=int,
        default=112
    )
...
def visualize(args):
    ...
    if args.input_type == 'resize':
        resize_transform = Resize((args.image_size, args.image_size, args.image_size))
    else:
    ...
    data_transform = Compose(
        [
            ...
            Lambda(func=lambda x: (x - x.mean()) / (x.std() + 1e-8)),  # zscore normalization
    ...
    if args.input_type == 'full_image' or args.input_type == 'resize':
        ...
        output = sliding_window_inference(img.unsqueeze(0), (112, 112, 112), 4, make_patchwise_predictor(model_forward), overlap=0.)
    else:
