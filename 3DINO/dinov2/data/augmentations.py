...
class DataAugmentationDINO3d(object):
    ...
    def __init__(
        ...
        local_transfo_extra = RandGaussianSmooth(prob=0.01)

        ## AA CHANGES
        intensity_transform1 = RandScaleIntensity(factors=(1/1.1, 1.1), prob=1.0)
        intensity_transform2 = RandShiftIntensity(offsets=0.1, safe=False, prob=1.0)
        ...
        self.global_transfo1 = Compose([gaussian_transforms, global_transfo1_extra, intensity_transform1, intensity_transform2, RandGaussianNoise(prob=1.0, std=0.002), ToTensor()])
        self.global_transfo2 = Compose([gaussian_transforms, global_transfo2_extra, intensity_transform1, intensity_transform2, RandGaussianNoise(prob=1.0, std=0.002), ToTensor()])
        self.local_transfo = Compose([gaussian_transforms, local_transfo_extra, intensity_transform1, intensity_transform2, RandGaussianNoise(prob=1.0, std=0.002),ToTensor()])
    ...
    def __call__(self, image):
        ...
        ### AA CHANGES
        # Debug: print only if NaN, Inf, or weird values detected
        # def check_tensor(name, t):
        #     nan_count = torch.isnan(t).sum().item()
        #     inf_count = torch.isinf(t).sum().item()
        #     total = t.numel()
        #     min_val, max_val = t.min().item(), t.max().item()
        #     is_weird = abs(min_val) > 100 or abs(max_val) > 100 or abs(max_val - min_val) < 1e-6
        #     if nan_count > 0 or inf_count > 0 or is_weird:
        #         print(f"[WARNING] {name}: min={min_val:.4f} max={max_val:.4f} nan={nan_count}/{total} ({100*nan_count/total:.1f}%) inf={inf_count}")
        #         return True
        #     return False

        # found_issue = False
        # for i, gc in enumerate(output["global_crops"]):
        #     if check_tensor(f"global_crop_{i}", gc):
        #         found_issue = True
        # for i, lc in enumerate(output["local_crops"]):
        #     if check_tensor(f"local_crop_{i}", lc):
        #         found_issue = True
        # if found_issue:
        #     print(f"  input image: min={image.min():.4f} max={image.max():.4f} shape={image.shape}")
        ### AA CHANGES END
