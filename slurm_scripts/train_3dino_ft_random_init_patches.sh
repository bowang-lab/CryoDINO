#!/bin/bash
#SBATCH -J 3dino-ft-random-init-patches-512
...
# =========================
# Fine-tuning with random initialization (no pretraining)
# Pre-extracted 512^3 patches (.pt), crops per patch, batch size 2
# Random ViT + ViTAdapter-UNETR decoder
# All 4 datasets: Dataset001, Dataset010, Dataset989, Dataset049
# =========================
...
PRETRAINED_WEIGHTS=""   # empty = random initialization
...
INFER_DATASET_NAME["Dataset049_EMPIAR_12049_transposed_patches512"]="Dataset049_EMPIAR_6class"   # avoids 12049 remap
...
for DATASET_NAME in "${DATASETS[@]}"; do
    ...
    OUTPUT_DIR="${BASE_OUTPUT_DIR}/random_init_${DATASET_NAME}_vit_adapter"
    # Reuse existing cache (same data, transforms are identical)
    ...
    echo "=============================================="
    echo "Starting 3D segmentation fine-tuning (random init)..."
